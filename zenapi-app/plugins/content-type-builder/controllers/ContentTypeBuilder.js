...
module.exports = {
  ...
  getModel: async ctx => {
    const Service = zenapi.plugins['content-type-builder'].services.contenttypebuilder;
    ...
    if (!source && !_.get(zenapi.models, model)) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.model.unknown' }] }]);

    if (source && !_.get(zenapi.plugins, [source, 'models', model])) {
  ...
  createModel: async ctx => {
    ...
    if (zenapi.models[name]) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.model.exist' }] }]);
    ...
    try {
      ...
      try {
        ...
        if (_.isEmpty(zenapi.api)) {
          zenapi.emit('didCreateFirstContentType');
        } else {
          zenapi.emit('didCreateContentType');
        }
        
        ctx.send({ ok: true });
        
        zenapi.log.debug('RESTART - createModel');
        //zenapi.reload();
        zenapi.restartProcess();
        const _zenapi = zenapi;
        setTimeout(() => {
          _zenapi.reload();
        }, 9000);
        //setImmediate(() => zenapi.reload(true));
      } catch (e) {
        zenapi.emit('didNotCreateContentType', e);
  ...
  updateModel: async ctx => {
    ...
    if (!name) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.name.missing' }] }]);
    if (!_.includes(Service.getConnections(), connection)) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.connection.unknow' }] }]);
    if (zenapi.models[_.toLower(name)] && name !== model) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.model.exist' }] }]);
    if (!zenapi.models[_.toLower(model)] && plugin && !zenapi.plugins[_.toLower(plugin)].models[_.toLower(model)]) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.model.unknown' }] }]);
    if (!_.isNaN(parseFloat(name[0]))) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.model.name' }] }]);
    if (plugin && !zenapi.plugins[_.toLower(plugin)]) return ctx.badRequest(null, [{ message: [{ id: 'request.error.plugin.name' }] }]);
    ...
    try {
      ...
      try {
        ...
        zenapi.log.debug('RESTART - updateModel');
  ...
  deleteModel: async ctx => {
    ...
    if (!_.get(zenapi.models, model)) return ctx.badRequest(null, [{ messages: [{ id: 'request.error.model.unknown' }] }]);
    ...
    zenapi.log.debug('RESTART - deleteModel');
    //zenapi.reload();
    zenapi.restartProcess();
    const _zenapi = zenapi;
    setTimeout(() => {
  ...
  autoReload: async ctx => {
    ctx.send({
      autoReload: _.get(zenapi.config.currentEnvironment, 'server.autoReload', { enabled: false })
    });
  },
  ...
  checkTableExists: async ctx => {
    ...
    const connector = _.get(zenapi.config.currentEnvironment.database.connections, [connection, 'connector']);
    ...
    if (connector === 'zenapi-hook-bookshelf') {
      try {
        const tableExists = await zenapi.connections[connection].schema.hasTable(model);
