...
export function* deleteModel({ context: { plugins, updatePlugin }, modelName }) {
  try {
    ...
    if (response.ok === true) {
      zenapi.notification.success(`${pluginId}.notification.success.contentTypeDeleted`);
    ...
    zenapi.notification.refresh(`${pluginId}.notification.restart`);
    setTimeout(() => {
      // Reload Browser
...
export function* submitCT({
  ...
  try {
    ...
    zenapi.notification.error(errorMessage);
  }
}
