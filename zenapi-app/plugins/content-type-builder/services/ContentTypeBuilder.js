...
const generator = require('zenapi-generate');
...
module.exports = {
  ...
  getModels: () => {
    ...
    _.forEach(zenapi.models, (model, name) => {
      if (name === 'core_store') {
        return true;
      }
    ...
    const pluginModels = Object.keys(zenapi.plugins).reduce((acc, current) => {
      _.forEach(zenapi.plugins[current].models, (model, name) => {
        if (name === 'file') {
          return true;
  ...
  getModel: async (name, source) => {
    ...
    const model = source ? _.get(zenapi.plugins, [source, 'models', name]) : _.get(zenapi.models, name);
    ...
    //zenapi.log.debug('getModel', name);
      ...
      if (relation && !_.isArray(_.get(relation, relation.alias))) {
        if ((params.plugin === 'upload' && relation.model) || relation.collection === 'file') {
          ...
          params.targetColumnName = _.get(
            (params.plugin ? zenapi.plugins[params.plugin].models : zenapi.models)[params.target].attributes[
              params.key
            ],
            'columnName',
            '',
  ...
  getConnections: () => {
    return _.keys(zenapi.config.currentEnvironment.database.connections);
  },

  generateAPI: (name, description, connection, collectionName, attributes) => {
    const template = _.get(
      zenapi.config.currentEnvironment,
    ...
    return new Promise((resolve, reject) => {
      const scope = {
        generatorType: 'api',
        id: name.toLowerCase(),
        rootPath: zenapi.config.appPath,
        args: {
          api: name,
  ...
  getModelPath: (model, plugin) => {
    ...
    const target =
      Object.keys((plugin ? zenapi.plugins : zenapi.api) || {}).filter(x =>
        _.includes(
          Object.keys(_.get((plugin ? zenapi.plugins : zenapi.api)[x], 'models', [])),
          model.toLowerCase(),
        ),
      )[0] || model.toLowerCase();
    ...
    const filename = fs
      .readdirSync(
        plugin
          ? path.join(zenapi.config.appPath, 'plugins', target, 'models')
          : path.join(zenapi.config.appPath, 'api', target, 'models'),
      )
      .filter(x => x[0] !== '.')
      .filter(x => x.split('.settings.json')[0].toLowerCase() === model.toLowerCase())[0];

    return plugin
      ? path.resolve(zenapi.config.appPath, 'plugins', target, 'models', filename)
      : path.resolve(zenapi.config.appPath, 'api', target, 'models', filename);
  },
  ...
  formatAttributes: (attributes, name, plugin) => {
    ...
    const target =
      Object.keys((plugin ? zenapi.plugins : zenapi.api) || {}).filter(x =>
        _.includes(Object.keys(_.get((plugin ? zenapi.plugins : zenapi.api)[x], 'models', [])), name),
      )[0] || name.toLowerCase();

    const model =
      (plugin
        ? _.get(zenapi.plugins, [target, 'models', name])
        : _.get(zenapi.api, [target, 'models', name])) || {};
    ...
    _.forEach(attributesConfigurable, attribute => {
      if (_.has(attribute, 'params.type')) {
        ...
        if (attribute.params.type === 'media') {
          const via = _.findKey(zenapi.plugins.upload.models.file.attributes, { collection: '*' });
  ...
  clearRelations: (model, source, force) => {
    const errors = [];
    const structure = {
      models: zenapi.models,
      plugins: Object.keys(zenapi.plugins).reduce((acc, current) => {
        acc[current] = {
          models: zenapi.plugins[current].models,
        };
    ...
    const deleteAssociations = (models, plugin) => {
      Object.keys(models).forEach(name => {
        const relationsToDelete = _.get(
          plugin ? zenapi.plugins[plugin].models[name] : zenapi.models[name],
          'associations',
          [],
        ).filter(association => {
          if (source) {
        ...
        if (!_.isEmpty(relationsToDelete)) {
          // Retrieve where is located the model.
          const target = Object.keys((plugin ? zenapi.plugins : zenapi.api) || {}).filter(x =>
            _.includes(Object.keys(_.get((plugin ? zenapi.plugins : zenapi.api)[x], 'models', [])), name),
          )[0];
