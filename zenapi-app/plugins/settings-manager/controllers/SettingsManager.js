...
module.exports = {
  menu: async ctx => {
    const Service = zenapi.plugins['settings-manager'].services.settingsmanager;
  ...
  databaseModel: async ctx => {
    const Service = zenapi.plugins['settings-manager'].services.settingsmanager;
    const env = zenapi.config.environment;
  ...
  createLanguage: async ctx => {
    ...
    const availableLanguages = zenapi.plugins['settings-manager'].services.languages;
    ...
    const filePath = path.join(zenapi.config.appPath, 'config', 'locales', `${name}.json`);
  ...
  createDatabase: async ctx => {
    ...
    if (_.isEmpty(_.keys(zenapi.config.environments[env].database.connections)) || _.isEmpty(zenapi.config.environments[env].database.defaultConnection)) {
  ...
  updateDatabase: async ctx => {
    ...
    if (params.database.connections) {
      const settings = _.assign(_.clone(zenapi.config.environments[env].database.connections[name].settings), params.database.connections[name].settings);
      const options = _.assign(_.clone(zenapi.config.environments[env].database.connections[name].options), params.database.connections[name].options);
      params = _.assign(_.clone(zenapi.config.environments[env].database.connections[name]), params.database.connections[name]);
    ...
    const connections = _.clone(zenapi.config.environments[env].database.connections);
    ...
    if (newName && newName !== name) {
      ...
      _.forEach(zenapi.models, (model, modelName) => {
    ...
    if (newName && newName !== name && zenapi.config.environments[env].database.defaultConnection === name) {
  ...
  deleteDatabase: async ctx => {
    ...
    if (zenapi.config.environments[env].database.defaultConnection === name) {
