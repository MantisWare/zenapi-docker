  ...
  try {
    ...
    if (indexDataToDelete !== -1) {
      ...
      if (response.ok) {
        ...
        zenapi.notification.success('users-permissions.notification.success.delete');
      }
    }
  } catch(err) {
    zenapi.notification.error('users-permissions.notification.error.delete');
  }
}
...
export function* dataFetch(action) {
  try {
    ...
    zenapi.notification.error('users-permissions.notification.error.fetch');
  }
}
...
export function* submitData(action) {
  try {
    ...
    zenapi.notification.success('users-permissions.notification.success.submit');
  } catch(error) {
