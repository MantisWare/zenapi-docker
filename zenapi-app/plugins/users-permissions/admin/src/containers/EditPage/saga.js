...
export function* fetchUser(action) {
  try {
    ...
    zenapi.notification.error('users-permissions.notification.error.fetchUser');
  }
}
...
export function* permissionsGet() {
  try {
    const response = yield call(request, '/users-permissions/permissions', {
      method: 'GET',
      params: {
        lang: zenapi.currentLanguage,
      },
    });

    yield put(getPermissionsSucceeded(response));
  } catch(err) {
    zenapi.notification.error('users-permissions.EditPage.notification.permissions.error');
  }
}
...
export function* policiesGet() {
  try {
    ...
    zenapi.notification.error('users-permissions.EditPage.notification.policies.error');
  }
}
...
export function* roleGet(action) {
  try {
    ...
    zenapi.notification.error('users-permissions.EditPage.notification.role.error');
  }
}
