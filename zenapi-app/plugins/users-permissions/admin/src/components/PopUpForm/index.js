...
class PopUpForm extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  getRedirectURIProviderConf = () => { // NOTE: Still testings providers so the switch statement is likely to change
    switch (this.props.dataToEdit) {
      case 'discord':
        return `${zenapi.backendURL}/connect/discord/callback`;
      case 'facebook':
        return `${zenapi.backendURL}/connect/facebook/callback`;
      case 'google':
        return `${zenapi.backendURL}/connect/google/callback`;
      case 'github':
        return get(this.props.values, 'redirect_uri', '');
      case 'microsoft':
        return `${zenapi.backendURL}/connect/microsoft/callback`;
      case 'twitter':
        return `${zenapi.backendURL}/connect/twitter/callback`;
      default: {
        const value = get(this.props.values, 'callback', '');

        return startsWith(value, 'http') ? value : `${zenapi.backendURL}${value}`;
      }
    }
  }

  generateRedirectURL = (url) => {
    return startsWith(url, 'https://') || startsWith(url, 'http://') || this.state.isEditing ? url : `${zenapi.backendURL}${startsWith(url, '/') ? '' : '/'}${url}`;
  }
  ...
  renderForm = () => {
    ...
    const params = {
      link: (
        <a href="https://github.com/tysflotv/zenapi/blob/master/packages/zenapi-plugin-users-permissions/docs/email-templates.md" target="_blank">
