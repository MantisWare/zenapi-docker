...
module.exports = async cb => {
  ...
  const pluginsLayout = Object.keys(zenapi.plugins).reduce((acc, current) => {
    const models = _.get(zenapi.plugins, [current, 'config', 'layout'], {});
  ...
  const tempLayout = Object.keys(zenapi.models)
  ...
  const models = _.mapValues(zenapi.models, pickData);
  delete models['core_store'];
  const pluginsModel = Object.keys(zenapi.plugins).reduce((acc, current) => {
    acc[current] = {
      models: _.mapValues(zenapi.plugins[current].models, pickData),
    };
  ...
  }, Object.keys(zenapi.models).filter(m => m !== 'core_store'));
  // Init schema
  const schema = {
    generalSettings: {
      search: true,
  ...
  try {
    ...
    schemaApis.map(model => {
      ...
      _.set(
        ...
        'info',
        _.get(!isPlugin ? zenapi.models[model] : zenapi[model], 'info')
      );
      _.set(
        prevSchema.models[model],
        'options',
        _.get(!isPlugin ? zenapi.models[model] : zenapi[model], 'options')
      );
    });
