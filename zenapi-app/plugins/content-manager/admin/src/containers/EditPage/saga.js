...
function* dataGet(action) {
  try {
    ...
    zenapi.notification.error('content-manager.error.record.fetch');
  }
}
...
function* deleteData() {
  try {
    ...
    zenapi.notification.success('content-manager.success.record.delete');
    ...
    zenapi.notification.error('content-manager.error.record.delete');
  }
}
...
export function* submit(action) {
  ...
  try {
    ...
    zenapi.notification.success('content-manager.success.record.save');
    ...
    zenapi.notification.error(`${notifErrorPrefix}${get(err.response, ['payload', 'message', '0', 'messages', '0', 'id'], isCreating ? 'content-manager.error.record.create' : 'content-manager.error.record.update')}`);
  } finally {
