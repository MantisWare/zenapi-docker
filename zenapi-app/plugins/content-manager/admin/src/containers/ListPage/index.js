...
// ./node_modules/zenapi-helper-plugin/lib/src
// or zenapi/packages/zenapi-helper-plugin/lib/src
  ...
  handleChangeHeader = ({ target }) => {
    ...
    if (attrIndex !== -1) {
      if (get(this.props.listPage, 'displayedFields', []).length === 1) {
        zenapi.notification.error('content-manager.notification.error.displayedFields');
      } else {
...
const withReducer = zenapi.injectReducer({ key: 'listPage', reducer, pluginId });
const withSaga = zenapi.injectSaga({ key: 'listPage', saga, pluginId });
