...
class SettingPage extends React.PureComponent {
  ...
  handleChange = e => {
    ...
    if (isDisablingDefaultSort) {
      ...
      if (enableAttrsSort.length === 0) {
        zenapi.notification.info('content-manager.notification.info.SettingPage.disableSort');
      } else {
  ...
  manageRemove = (index, keys, itemName, data, isRelation = true) => {
    ...
    if (isRemovingSelectedItem && displayedRelations.length === 1 && allFields.length === 0) {
      return zenapi.notification.info('content-manager.notification.error.displayedFields');
    }
...
const withReducer = zenapi.injectReducer({ key: 'settingPage', reducer, pluginId });
const withSaga = zenapi.injectSaga({ key: 'settingPage', saga, pluginId });
